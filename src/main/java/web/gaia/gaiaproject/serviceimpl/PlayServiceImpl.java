            ...
            for (int j=1;j<=7;j++){
                ...
                if(play == null) {result[i+1][j*2] = "";}else {
                    ...
                    if(game.getBlackstar()!=null&&game.getBlackstar().equals("游戏结束")){
                        ...
                        if(win==1&&draw==2) {result[i+1][16] = String.valueOf(Float.parseFloat(result[i+1][16])+0.33);result[i+1][17] = String.valueOf(Float.parseFloat(result[i+1][17])+0.33);result[i+1][18] = String.valueOf(Float.parseFloat(result[i+1][18])+0.33);result[i+1][21] = String.valueOf(Float.parseFloat(result[i+1][21])+3.33);}
